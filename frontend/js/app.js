document.addEventListener('DOMContentLoaded', function () {
  ...
  // API Configuration
  const FORMAT_API_ENDPOINT = 'http://localhost:8000/app/v1/youtube';
  ...
  async function handleVideoOptions(url) {
    try {
      ...
      const response = await fetch(`${FORMAT_API_ENDPOINT}/formats`, {
        method: 'POST',
        headers: {
  ...
  async function handleDownload(url) {
    ...
    try {
      ...
      const downloadResponse = await fetch(`${FORMAT_API_ENDPOINT}/download/video`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  function createDownloadButton(url) {
    ...
    button.addEventListener('click', () => handleDownload(url));
...
// Add handleAudioOptions function (implement according to your audio download requirements)
  function handleAudioOptions(url) {
    // Implement audio download logic here
    console.log('Audio download for:', url);
  }

  // Display video format and quality options based on API response.
